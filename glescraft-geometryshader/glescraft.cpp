...
struct chunk {
	...
	int8_t intensity(int y) {
		int ry = y + ay * CY;
		float val = exp((ry - 3) / 5.0);
		if(val > 1)
			val = 1;
		return lrintf(127 * val);
	}
			...
			for(int y = 0; y < CY; y++) {
				for(int z = 0; z < CZ; z++) {
					...
					// Otherwise, add a new quad.
						...
						vertex[i++] = byte4(x, y, z + 1, intensity(y));
					}
			...
			for(int y = 0; y < CY; y++) {
				for(int z = 0; z < CZ; z++) {
					...
					if(vis && z != 0 && blk[x][y][z] == blk[x][y][z - 1]) {
						...
						vertex[i++] = byte4(x + 1, y + 1, z + 1, intensity(y));
					}
					vis = true;
				}
			}
		...
		for(int x = 0; x < CX; x++) {
			for(int y = CY - 1; y >= 0; y--) {
				for(int z = 0; z < CZ; z++) {
					...
					if(vis && z != 0 && blk[x][y][z] == blk[x][y][z - 1]) {
						vertex[i - 1] = byte4(x + 1, y, z + 1, intensity(y));
						merged++;
					} else {
						vertex[i++] = byte4(x, y, z, bottom);
						vertex[i++] = byte4(x + 1, y, z + 1, intensity(y));
					}
					vis = true;
				}
			}
			...
			for(int y = 0; y < CY; y++) {
				for(int z = 0; z < CZ; z++) {
					...
					if(vis && z != 0 && blk[x][y][z] == blk[x][y][z - 1]) {
						...
						vertex[i++] = byte4(x, y + 1, z + 1, intensity(y));
					}
					vis = true;
				}
			}
		...
		for(int y = 0; y < CY; y++) {
			for(int z = CZ - 1; z >= 0; z--) {
				for(int x = 0; x < CX; x++) {
					...
					if(vis && x != 0 && blk[x][y][z] == blk[x - 1][y][z]) {
						vertex[i - 1] = byte4(x + 1, y + 1, z, intensity(y));
						merged++;
					} else {
						vertex[i++] = byte4(x, y, z, side);
						vertex[i++] = byte4(x + 1, y + 1, z, intensity(y));
					}
					vis = true;
				}
			}
...
static void display() {
	...
	glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_MIN_FILTER, GL_NEAREST); // Use GL_NEAREST_MIPMAP_LINEAR if you want to use mipmaps
	...
	/* Render a box around the block we are pointing at.
           Small offsets are necessary to be able to reuse the same shaders as when rendering the chunks */

	float box[24][4] = {
		{bx + 0, by - 0.02f, bz + 0, 14},
		{bx + 1, by + 0.02f, bz + 0, 127},
		{bx + 0, by + 0.98f, bz + 0, 14},
		{bx + 1, by + 1.02f, bz + 0, 127},
		{bx + 0, by - 0.02f, bz + 1, 14},
		{bx + 1, by + 0.02f, bz + 1, 127},
		{bx + 0, by + 0.98f, bz + 1, 14},
		{bx + 1, by + 1.02f, bz + 1, 127},

		{bx - 0.02f, by + 0, bz + 0, 14},
		{bx + 0.02f, by + 1, bz + 0, 127},
		{bx + 0.98f, by + 0, bz + 0, 14},
		{bx + 1.02f, by + 1, bz + 0, 127},
		{bx - 0.02f, by + 0, bz + 1, 14},
		{bx + 0.02f, by + 1, bz + 1, 127},
		{bx + 0.98f, by + 0, bz + 1, 14},
		{bx + 1.02f, by + 1, bz + 1, 127},

		{bx - 0.02f, by + 0, bz + 0, 14},
		{bx + 0.02f, by + 0, bz + 1, 127},
		{bx + 0.98f, by + 0, bz + 0, 14},
		{bx + 1.02f, by + 0, bz + 1, 127},
		{bx - 0.02f, by + 1, bz + 0, 14},
		{bx + 0.02f, by + 1, bz + 1, 127},
		{bx + 0.98f, by + 1, bz + 0, 14},
		{bx + 1.02f, by + 1, bz + 1, 127},
	};
